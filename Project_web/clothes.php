...
<body>
                ...
                <div class="home-product">
                    ...
                    <?php
                        ...
                        if (isset($_GET['q'])) {
                            ...
                            // Modify the SQL query to include the search functionality and filter by ID_DM
                            $sql = "SELECT sanpham.*, danhmucsanpham.TEN_DM 
                                    FROM sanpham 
                                    INNER JOIN danhmucsanpham ON sanpham.ID_DM = danhmucsanpham.ID_DM
                                    WHERE (sanpham.TEN_SP LIKE '%$search_query%' OR danhmucsanpham.TEN_DM LIKE '%$search_query%') AND sanpham.ID_DM = 101";
                        } else {
                            // Default query without search and filter by ID_DM
                            $sql = "SELECT sanpham.*, danhmucsanpham.TEN_DM FROM sanpham INNER JOIN danhmucsanpham ON sanpham.ID_DM = danhmucsanpham.ID_DM WHERE sanpham.ID_DM = 101";
                        }
                        ...
                        if ($ketqua->num_rows > 0) {
                            while ($row = $ketqua->fetch_assoc()) {
                                ...
                                echo '<a href="infproduct.php?id=' . $row['ID_SP'] . '" class="home-product-item__link">';
                                ...
                                // // Thêm sự kiện onclick để gọi hàm thêm sản phẩm vào giỏ hàng
                                // echo '<i class="fa-solid fa-cart-shopping" onclick="addToCart(' . $row['ID_SP'] . ', \'' . $row['TEN_SP'] . '\', ' . $row['GIA'] . ')"></i>';
                                // echo '</div>';
                                echo '</div>';
                                echo '</div>';
                                echo '</a>';
    ...
    function addToCart(productID, productName, productPrice) {
        // Gửi yêu cầu AJAX đến tệp PHP xử lý thêm sản phẩm vào giỏ hàng
        ...
        xhr.open("GET", "add_to_cart.php?id=" + productID + "&name=" + encodeURIComponent(productName) + "&price=" + productPrice, true);
        xhr.send();
    }
</script> -->
</body>
